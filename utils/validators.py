...
def is_valid_phone_number(phone_number: str) -> bool:
    """Checks if the given string is a valid phone number format (basic check)."""
    if not isinstance(phone_number, str):
        return False
    # Allows digits, spaces, hyphens, parentheses, and plus sign (for international)
    pattern = r"^\+?[0-9\s-]+$"
...
def is_positive_number(value) -> bool:
    """Checks if a value is a positive number (int or float)."""
    try:
        num = float(value)
        return num > 0
...
def is_non_empty_string(s: str) -> bool:
    """Checks if a string is not None and not empty after stripping whitespace."""
